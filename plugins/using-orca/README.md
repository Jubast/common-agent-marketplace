...
Teaches Claude Code to recognize and require the `orca-cli` and
`orchestration` skills for worktree/agent-dispatch work in Orca-managed
environments.

## What's in here

- `.claude-plugin/plugin.json` — plugin manifest.
- `skills/using-orca/` — a short trigger skill pointing at `orca-cli` and
  `orchestration` for their full command references.
...
- A pointer, not a reference — see `orca-cli`/`orchestration` for full
  capabilities.
