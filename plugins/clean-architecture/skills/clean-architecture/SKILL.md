...
description: 'Use only when Robert C. Martin''s Clean Architecture principles are explicitly invoked for C#/.NET code or solution structure — phrases like "clean architecture", "apply the dependency rule", "hexagonal architecture", "onion architecture", "ports and adapters", "screaming architecture", "is this layered correctly", "Uncle Bob''s architecture" — or a direct ask to check the dependency rule, SOLID/component principles, entities vs. use cases, interface adapters, boundaries, frameworks-as-details, solution structure, or testing strategy against the book. Covers design, writing, and review of C#/.NET solution structure against a curated subset of the book (excludes programming-paradigms, embedded-architecture, and case-study chapters). Does NOT trigger on generic "review this code", "write a function that...", or generic project scaffolding requests with no Clean Architecture framing — code-review, simplify, and test-driven-development already cover those.'
---
