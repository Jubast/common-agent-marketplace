...
1. **File it.**
   `${CLAUDE_PLUGIN_ROOT}/bin/chief-backlog.sh add <id> "<short title>"`
...
6. **If it's stuck**, escalate cheapest first:
   ...
   - `${CLAUDE_PLUGIN_ROOT}/bin/chief-control.sh <id> relaunch --note "<progress so far>"` only if genuinely wedged - the replacement gets the same worktree and commits but none of the conversation, so the note is all it has.
...
- **Ready to land** (ship only) - `${CLAUDE_PLUGIN_ROOT}/bin/chief-pr-open.sh <id> --confirm` to push and open a PR/MR. Skip if they want a local-only merge.
...
`${CLAUDE_PLUGIN_ROOT}/bin/chief-backlog.sh add|status|note|hold|done|list|next|show <id> ...` - one markdown file (`.chief/data/backlog.md`), statuses are `queued|in-flight|held|done`. Use `hold <id> "<reason>"` for anything that needs an operator decision before it can proceed.
