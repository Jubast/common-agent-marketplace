...
# touches the real, possibly-shared ~/.claude.json.
set -euo pipefail
...
if ! HOME="$TEST_HOME" claude plugin marketplace add "$REPO_ROOT"; then
...
while IFS= read -r plugin_name; do
    ...
    if ! HOME="$TEST_HOME" claude plugin install "$plugin_name@$MARKETPLACE_NAME" -y; then
        echo "  [FAIL] install failed: $plugin_name"
        overall_fail=$((overall_fail + 1))
        continue
    fi

    list_output=$(HOME="$TEST_HOME" claude plugin list 2>&1)
